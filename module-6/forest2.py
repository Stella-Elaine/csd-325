...
def createLake(forest):
    """Creates a lake in the center of the forest."""
    # Calculate lake boundaries
    start_x = LAKE_CENTER_X - (LAKE_WIDTH // 2)
    start_y = LAKE_CENTER_Y - (LAKE_HEIGHT // 2)
    
    # Place the lake
    for x in range(start_x, start_x + LAKE_WIDTH):
        for y in range(start_y, start_y + LAKE_HEIGHT):
            if 0 <= x < WIDTH and 0 <= y < HEIGHT:  # Ensure we're within bounds
                forest[(x, y)] = WATER
...
def main():
    ...
    forest = createLake(forest)  # Add the lake after creating the forest
    bext.clear()
